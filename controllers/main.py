...
class WebsiteEventTicketStore(WebsiteSale):
    ...
    def shop_payment_validate(self, sale_order_id=None, **post):
        ...
        if order and not order.amount_total and not tx_sudo:
            # Check if this is an event order that needs attendee collection FIRST
            # This must happen before validation to prevent auto-confirmation
            if order.order_line.filtered(lambda line: line.product_id.service_tracking == 'event'):
                ...
                if not has_registrations:
                    # Generate access token for the order (like paid orders)
                    token = order._generate_attendee_access_token()
                    # Send email reminder
                    self._send_attendee_details_reminder(order)
                    # Redirect to token-based URL (same as paid orders)
                    return request.redirect(order.get_attendee_details_url())

            # For non-event orders or event orders with existing registrations, validate and proceed
            if order.state != 'sale':
    ...
    def order_attendee_details(self, order_id, access_token, **kw):
        ...
        is_free_order = not order.amount_total
        has_valid_payment = tx and tx.state in ['done', 'authorized']
        
        if not is_free_order and not has_valid_payment:
